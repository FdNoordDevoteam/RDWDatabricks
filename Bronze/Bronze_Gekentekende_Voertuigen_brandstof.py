...
file_name = "Gekentekende_Voertuigen_Brandstof"
...
# %sql
# Create table gekentekende_voertuigen_brandstof_delta_b
# ()
# Location '/mnt/iotdata/Gekentekende_Voertuigen_Brandstof/table/tbl_Gekentekende_Voertuigen_Brandstof_Bd'
...
joinDF = file_bronze_dump.join(file_B_DF,((file_bronze_dump.kenteken==file_B_DF.target_kenteken) & (file_bronze_dump.brandstof_omschrijving ==file_B_DF.target_brandstof_omschrijving)), "leftouter").select(file_bronze_dump["*"], file_B_DF["*"])
display(joinDF)
...
mergeDF = filterDF.withColumn("MERGEKEY", concat(filterDF.kenteken,filterDF.brandstof_omschrijving))
display(mergeDF)
...
condition = f"concat(target.kenteken, target.brandstof_omschrijving) = source.MERGEKEY and target.End_Date is null"
